    ...
    element = element->NextSiblingElement())
  {
    ...
    if (elementType == "STATION") {
      ...
      if(type == NULL) {
        ...
        endResult = PartialImport;
				deleted = true;
        // Temporary to log all errors
        station = new Halte;
      }
      else {
        std::string initType;
        if(element->FirstChildElement("type")->FirstChild() != NULL)
          initType = element->FirstChildElement("type")->FirstChild()->ToText()->Value();
        if(initType == "Metrostation") station = new MetroStation;
        // if type in invalid, will be deleted later on
        else station = new Halte;
      }
      if(SPOOR == NULL) {
        ...
        infoElem = infoElem->NextSiblingElement())
      {
        ...
        if(infoElem->FirstChild() == NULL) {
          ...
          deleted = true;
				...
				else if (elemName != "SPOOR" && elemName != "naam" && elemName != "type") {
              ...
              if (elemNameSpoor == "spoor") {
                try {
                  int nummer = std::stoi(text->Value());
									if (nummer >= 0) {
										spoorTemp = nummer;
										std::set<int> sporen = station->getSporen();
										if (sporen.find(nummer) != sporen.end()) {
											endResult = PartialImport;
											errStream << "XML PARTIAL IMPORT: Station not created, dubble line number: " << text->Value() << "." << std::endl;
											deleted = true;
											break;
										}
									}
      ...
      if(type == NULL) {
        ...
        endResult = PartialImport;
				deleted = true;
        // Temporary init to log all errors, will always be deleted
        tram = new PCC;
      }
      else 
      {
        ...
        if(element->FirstChildElement("type")->FirstChild() != NULL)
          initType = element->FirstChildElement("type")->FirstChild()->ToText()->Value();
        if(initType == "Albatros") tram = new Albatros;
        // if type in invalid, will be deleted later on
        else tram = new PCC;
      }
      if(voertuigNr == NULL) {
